...
By default, NestJS projects use port 3000 for the server. Let's change this to use an environment variable instead. This will be important for deployment as the hosting provider will inject the port number as an environment variable.

In the `main.ts` file, change the port number to use the `PORT` environment variable:
...
We've added a check to see if the `PORT` environment variable is set. If it's not, we'll default to port 3000.

Let's make one more tweak - let's update the server host to listen on `0.0.0.0`, a common practice for servers and common for most hosting providers. This will allow the server to listen on all network interfaces.

Throughout the course, we'll be deploying to [Railway](https://railway.app) which requires the server to listen on `0.0.0.0`.
...
Nice! There's finally one more thing to consider - what if the response is null or undefined? In that case, we can return the `data` property with an empty array, like this:
...
Finally, it's also worth updating the e2e test to reflect the new validation pipe:
...
Let's continue to the next section - building a custom logger!
...
Let's add a `User` model to the `schema.prisma` file so that we have a table to work with. In future projects, you'll likely remove/edit this model to suit your needs.
...
In fact, let's run it now to create the `User` table in the database. You'll be prompted to give the migration a name - just hit enter to use the default name.
...
One thing worth noting is the use of `any` in the `set` method. This is because the `ttl` property is not defined in the `@types/cache-manager` package. This is a known issue and will hopefully be fixed soon.
...
In unit testing, a 'dependency' refers to a part of the system that your code relies on, like an external service or database. In the given example, the dependency is the HttpService, which is used to make an external API call to fetch the Pokémon details.
...
Let's finally update the `test:e2e` script to run the `db:migrate:test` script before running the tests:
...
Add the `DatabaseModule` to the `imports` array in the `AppModule`:
...
@Module({
  imports: [CoreModule, DatabaseModule], // <--- Add the DatabaseModule
...
Then finally, let's update the testing setup file to reset the database after each test:
...
In the default test file that NestJS generates called `app.e2e-spec.ts`, you'll see that the `beforeEach` hook is used to create a new instance of the `Test` object, and then each test uses `app.getHttpServer()` to get the instance of the NestJS application and make requests to it.
...
The thing is, in our setup file, we're already creating an app instance and initializing it. So we can just use this instance in our tests instead of creating a new instance of the `Test` object and getting the app instance from it.

We can also move the `app.useGlobalPipes` into the set up file so we don't have to repeat it in every test file:
...
beforeAll(async () => {
  ...
  app = moduleFixture.createNestApplication();
  app.useGlobalPipes(new ValidationPipe({ whitelist: true })); // <--- Add the global pipes
...
Now that we have our tests set up, let's set up a CI/CD pipeline in Github Actions to run them.
...
The CD pipeline will run any Prisma database migrations and deploy the app to [Railway](https://railway.app/) on every push to the `main` branch.

Let's start with the CI pipeline. In the root of your project, add a `.github` directory and the following sub directories and files:
...
You can see I've added a job called `unit-tests` which has the following steps:
...
And also another job called `e2e-tests` which has the following steps:

- The checkout action is used to fetch your repository's code, making it available for subsequent steps in the workflow.
- The build action points to a file (that we'll add below) that installs the dependencies in the repository
- And then finally, the command `pnpm test:e2e` is used to run the e2e tests and integration tests

The `e2e-tests` job also has some environment variables defined. These are the credentials for the test database - if you look in the `docker-compose-test.yml` file, you'll see that the Postgres service uses these credentials.

Let's now add the `action.yml` file in the `build` directory:

```yml:.github/actions/build/action.yml
name: 'Build'
...
The purpose of this action is to install the dependencies in the repository using pnpm. It's used by both the `unit-tests` and `e2e-tests` jobs.

If you push up your code to Github, you'll see that the CI pipeline will run the tests (and the tests should all pass if you've been following along!).

Let's now set up the CD pipeline where we will:

- Run any Prisma database migrations
- Deploy the app to Railway

Importantly, we also only want to the CD pipeline to run when the tests pass.

Add a new file called `deploy.yml` in the `.github/workflows` directory and add the following:
...
- The `on` property is used to specify when the workflow should run. In this case, it will run when the `Tests` workflow is completed.
- The `if` property is used to specify when the job should run. In this case, it will run when the `Tests` workflow is completed and the tests have passed and the branch is `main`.
- The `uses` property is used to fetch your repository's code, making it available for subsequent steps in the workflow.
- The `build` action points to a file (that we already added earlier) that installs the dependencies in the repository
- The command `pnpx prisma migrate deploy` is used to apply any database migrations
- The command `pnpm add --global @railway/cli` is used to install the Railway CLI
- The command `railway up` is used to deploy the app to Railway

You'll notice that the `DATABASE_URL` & `RAILWAY_TOKEN` environment variable is used . This is the database URL for the production database and a token from Railway to link the Github repository to the Railway project and environment.

We don't want to hardcode these in the workflow file, so we'll add them as secrets in the repository.

We'll get these values from Railway shortly.

It's worth noting that Railway does have a built-in way of deploying on every push to a branch, but I find it easier if the deployment is within Github Actions along with the tests and database migrations so you can easily see the entire CI/CD pipeline in one place (and therefore easily debug any issues that happen).

## Monorepo set up

There will be some projects ahead where we'll be using a monorepo set up.

Let's update our existing set up to support this using pnpm workspaces.

If you're coding along, create a new branch called `monorepo` and checkout to it:

```shell
git checkout -b monorepo
```

The monorepo will have an `apps` directory which will contain all the apps (NestJS app, NextJS app, etc), so let's create that now, including a directory for the NestJS app:

```shell
mkdir -p apps/backend
```

Now let's move all the files from the root of the project into the `backend` directory, except for the `.github` directory which we want to keep in the root of the project:

```shell
mv * .[^.]* ./apps/backend/
```

It's possible you might get some errors when running that command - if you do I would suggest moving them manually in your IDE.

The .git and .github directory will now be in the /backend directory, so let's move that back to the root of the project:
...
It's important the `.git` file is in the root of your directory. To double check it is indeed there, make sure you're in the root of the project and run this command to check all the files (including hidden files) in the directory:

```shell
ls -a
```

In the `package.json` file inside the `/backend` directory, update the name of the project to `backend`:
...
Back in the root of the project, create a new file called `pnpm-workspace.yaml` and add the following:
...
This file tells pnpm that the `apps` directory contains all the apps in the monorepo.

Then let's initialize in the root:

```shell
pnpm init
```

This will create a `package.json` in the root of the project.

Also in the root, add a `.gitignore` file and add the following:

```
node_modules
```

pnpm adds a node_modules directory in the root of the project, so we want to git to ignore this.

Why does pnpm add a node_modules directory in the root of the project? Well, it's because pnpm uses symlinks to link the dependencies in the apps to the dependencies in the root of the project. This means that if you have multiple apps in your monorepo, they can share the same dependencies, which saves disk space and speeds up installation times for local development.

Inside the `main.ts` file from the existing NestJS app (now in the `apps/backend` directory), update the port to `4000`:
...
async function bootstrap() {
  ...
  app.use(helmet())
  app.useGlobalPipes(new ValidationPipe({ whitelist: true }))
  const port = process.env.PORT ? parseInt(process.env.PORT) : 4000 // <--- Update the port to 4000
  const host = `0.0.0.0`
  await app.listen(port, host)
}
bootstrap()
...
By default, NestJS (which we'll be setting up shortly) uses port 3000, which is the same as NextJS (which we'll be adding in the next step).

So to prevent a port conflict in your local servers, simply change this to 4000 for now to listen to this port instead.

### NextJS 14 set up

Let's add a NextJS 14 application called `web`. You can call this whatever you like!
...
You might be prompted to install some peer dependencies. You should install the respective peer dependencies as prompted.
...
This was an error due to the TailwindCSS setup with PostCSS.

To fix this, I installed the latest version of Tailwind to the web project:

```
pnpm add tailwindcss@latest
```

Before running `pnpm dev`, delete the `.next` directory in the web project as this contains some cache.

So we now have 2 apps in our monorepo, NestJS and NextJS!

## Monorepo config updates and DX tips

Both our NestJS app and NextJS app have a `tsconfig.json` file, so let's create a `tsconfig.json` file at the root of the monorepo so centralise some shared TypeScript config.

Add the `tsconfig.json` file to the root of the monorepo:

```bash
touch tsconfig.json
```

Add the following. If you've used different names for your application directories, make sure they're used in the `paths` object. I've used `backend` and `web`:
...
```json:apps/backend/package.json
{
  ...
  "scripts": {
    ...
    "start": "nest start",
    "dev": "pnpm docker:start && nest start --watch", // updated from start:dev to dev
    // ...
  },
  // ...
}
...
Let's add some build, start and test scripts for both applications in the root package.json.

This will be helpful later when we deploy each project.
...
```json
{
  ...
  "scripts": {
    ...
    "test:e2e": "pnpm run --parallel test:e2e"
  },
  "keywords": [],
  "author": "",
  "license": "ISC"
...
The test scripts ensure that the Github actions workflow will work with the new monorepo set up.

Nice work! We now have a monorepo set up with 2 applications, a NestJS for the backend heavy lifting and a NextJS app for the frontend.
