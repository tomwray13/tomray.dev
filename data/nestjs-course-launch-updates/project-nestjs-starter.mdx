...
Now let's move all the files from the root of the project into the `backend` directory:
...
The .git directory will now be in the /backend directory, so let's move that back to the root of the project:
...
Let's add some build and start scripts for both applications in the root package.json.
