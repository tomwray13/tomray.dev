...
As we're using the `CommonModule` as a place to define globally applied config and services, let's add the logging middleware there:
...
We're going to apply the cache set inside the `CommonModule`, so go there and add the `CacheModule`:
...
Inside the `/common` directory, add a `/cache` directory and inside there add a file called `cache.service.ts`:
...
Finally, back in the `CommonModule`, add the `CacheService` to the providers and exports:
...
Nice! The next thing we need to do is setup the scripts to start the Docker services before all the tests are run and stop them after all the tests are run.
