...
Let's implement the simple logic to generate a random emoji. Update the existing `app.service.ts` file to the following:
...
Let's update the controller to use this service:
...
The generated `app.controller.spec.ts` will need updating as we've replaced the default method with our own:
...
We'll get into unit and e2e testing later on.

Spin up your local server with `pnpm dev:start` and make a request to `http://localhost:3000` and you should see a random emoji returned in the response.

Let's now dive into the NestJS Lifecycle.
...
Not to mention middleware, guards, pipes and filters are also classes.
...
You might be wondering though 🤔 - that decorator we've just implemented seems quite different to the NestJS decorators like `@Controller` and `@Get()`.
