...
Nice work! Let's add some unit tests to make sure the validation is working as expected.

Let's test the following scenarios:

- More files than the max file count should throw a `BadRequestException`
- Files under the max file count should return the files
- Files equal to the max file count should return the files

In the same directory as the `max-file-count.pipe.ts` file, add a new file called `max-file-count.pipe.spec.ts` and add the respective tests:

```ts
import { BadRequestException } from '@nestjs/common'
import { MaxFileCountValidationPipe } from './max-file-count.pipe'
...
Worth noting: The `BullAdaptor` didn't automatically handle the import in my IDE, I had to manually add it:
...
Before continuing, let's add a couple of unit tests for the 2 new methods we added to the `FileService`.

We want to test:

- `bufferToBase64()` - that it converts a Buffer to a base64 string
- `base64ToBuffer()` - that it converts a base64 string to a Buffer

You should already have a `file.service.spec.ts` file (generated when using the Nest CLI earlier). Let's update it to test these 2 methods:
...
export default () => ({
  ...
  googleCloud: {
    ...
    privateKey: process.env.GCS_PRIVATE_KEY
      ...
      : ``,
    buckets: {
      listingImages: process.env.GCS_BUCKET_LISTING_IMAGES,
    },
  },
})
```
...
- `GCS_BUCKET_LISTING_IMAGES` is the name of the bucket you created earlier
...
export class GoogleCloudService {
  ...
  getBucket(bucketId: string) {
    return this.storage.bucket(bucketId)
  }
}
```
...
- Created a new instance of the `Storage` class assigned to the private storage property, passing in the relevant credentials. The private `storage` property is now an instance of the `Storage` class, which we can use to interact with Google Cloud Storage.
- Created a `getBucket` method which returns a bucket instance. This is useful as we can now inject the `GoogleCloudService` into other services and use the `getBucket` method to get a bucket instance. This also easily allows us to mock the `getBucket` method in unit tests.

Let's now create a new service to handle the file uploads to Google Cloud.

You might be thinking - why not just add a method for uploading files to the `GoogleCloudService` we just created?

In order to upload files, we will need access to the `Storage` instance, meaning it's a dependency. For better testing and separation of concerns, I'm going to keep the `GoogleCloudService` as a wrapper around the `Storage` class and create a new service for handling the file uploads:

```bash
npx nest generate service services/google-cloud/storage
```

And in this `StorageService`, we can inject the `GoogleCloudService` and use it to upload files:
...
export class StorageService {
  private bucket = `bucket-name` // <-- add your bucket name here - TODO: update to get from environment variables
  constructor(private readonly googleCloudService: GoogleCloudService) {}

  async uploadFile(fileBuffer: Buffer, fileMimeType: string) {
    const bucket = this.googleCloudService.getBucket(this.bucket)
    const fileName = `randomFileName` // TODO: generate random filename
...
You'll notice there's a TODO in the method - we need to generate a random filename for the file. This is because Google Cloud Storage requires a unique filename for each file.

There's also another TODO: right now we're using a default bucket name, but we should update this so our `uploadFile` method can accept a bucket name as an argument. This will allow us to upload files to different buckets which is typically what you'd want to do in a production application.
...
Let's add some unit tests for the `UidService`. In the generated test file `uid.service.spec.ts`, let's add 2 tests:

1. should return a UID of default length when no length is provided
2. should return a UID of specified length
...
describe(`UidService`, () => {
  ...
  it(`should return a UID of default length when no length is provided`, () => {
    ...
    // Assuming default length is 11 for the uid library.
...
@Module({
  ...
  providers: [GoogleCloudService, StorageService],
...
Let's now do the final update and enforce a bucket to be passed in when using the `uploadFile` method. To do this, we'll inject the configService to access the bucket value(s) as well as use a TypeScript literal type like this:
...
export class StorageService {
  ...
  async uploadFile(fileBuffer: Buffer, fileMimeType: string, bucketLabel: `listingImages`) {
    const bucketId = this.configService.getOrThrow(`googleCloud.buckets.${bucketLabel}`)
    const bucket = this.googleCloudService.getBucket(bucketId)
    const fileName = this.uidService.generate()
    const file = bucket.file(`${fileName}`)
    await file.save(fileBuffer, {
      metadata: {
...
Before proceeding, let's add some unit tests for the `uploadFile` method. In the generated `storage.service.spec.ts` file, you should have:
...
If you run this test, it will fail due to missing dependencies. You can see in the `StorageService` that we're injecting 3 dependencies:

- `GoogleCloudService`
- `UidService`
- `ConfigService`

Let's first mock these and fix the dependency issue. We'll pass in a fake value for the bucket value using the ConfigModule and use the `@golevelup/ts-jest` package to mock the `GoogleCloudService` and `UidService`.
        ...
        ConfigModule.forRoot({
          load: [
            () => ({
              googleCloud: {
                buckets: {
                  listingImages: `listing-images-bucket`,
                },
              },
            }),
          ],
...
Let's now add a unit test for this method - we want to test the end result of the method - it returns a public url for the file:
...
describe('StorageService', () => {
  ...
  let googleCloud: DeepMocked<GoogleCloudService>
  ...
  beforeEach(async () => {
    ...
    googleCloud = module.get(GoogleCloudService)
  })
  ...
  it(`should return a public url`, async () => {
    ...
    const mockBucketId = `listingImages`
    const returnedPublicUrl = `https://storage.googleapis.com/listing-images-bucket/random-uid`

    uidService.generate.mockReturnValue(`random-uid`)
    const bucket = googleCloud.getBucket(`listing-images-bucket`)
    bucket.file.mockReturnValue({
      save: jest.fn(),
      makePublic: jest.fn(),
      publicUrl: jest.fn().mockReturnValue(returnedPublicUrl),
    } as any)

    const publicUrl = await storageService.uploadFile(mockBuffer, mockMimeType, mockBucketId)
    expect(publicUrl).toEqual(returnedPublicUrl)
  })
})
```
...
- We're mocking the `UidService` and `GoogleCloudService` using the `@golevelup/ts-jest` package
- We're mocking the `generate` method of the `UidService` to return a static value
- We're mocking the `getBucket` method of the `GoogleCloudService` to return a mock bucket instance
- We're mocking the `file` method of the bucket instance to return a mock file instance
- We're mocking the `save`, `makePublic` and `publicUrl` methods of the file instance to return a mock public url
- We're then passing in some values to the `uploadFile` method and asserting that the public url returned is the same as the mock public url

Due to the amount of mocking happening (and the test brittleness that this results in), the value of this unit test is questionable. I've included it to show you an example with quite a bit of mocking.

Moving on, let's go back to the consumer and use this service to upload the file to Google Cloud Storage:
...
import { BaseConsumer } from '../../../core/queue/base.consumer'
import { FileService } from '../../../utilities/file/file.service'
import { StorageService } from '../../../services/google-cloud/storage/storage.service'
import { UploadListingImageDto } from '../dto/upload-listing-image.dto'
...
export class ListingConsumer extends BaseConsumer {
  constructor(
    logger: Logger,
    private readonly fileService: FileService,
    private readonly googleCloudStorage: StorageService
  ) {
    super(logger)
  }
  ...
  async uploadListingImage(job: Job<UploadListingImageDto>) {
    ...
    const buffer = this.fileService.base64ToBuffer(base64File)
    const publicUrl = await this.googleCloudStorage.uploadFile(buffer, mimeType, `listingImages`)
...
Let's write some unit, integration and end-to-end tests for the logic we've added.

We've already added some unit tests for the various pipes and services we've created except for the `ListingService`. So let's add some unit tests for the `ListingService`.

If it doesn't exist, add a `listing.service.spec.ts` file in the `listing` directory and add the following:
...
The `should be defined` test fail as the `ListingService` has the following dependencies:
...
Let's update the module in the test to automatically mock these dependencies using `createMock` from the `@golevelup/ts-jest` package:
...
The test should be passing now as the dependencies are mocked.

Let's consider what we want to unit test for the `create` and `uploadImage` in the `ListingService`. For unit tests, we'll be focused on the end result of the method, not the implementation details.

For the `create` method, we want to test that the method returns the created listing from the data passed into the DTO. We'll need to mock the `DatabaseService` to return a mock listing:
...
The `DatabaseService` is an @Injectable class but it's also a Prisma Client instance. So we need to use the `mockDeep` method from the `jest-mock-extended` package to create a test double. This method takes in the class you want to mock and returns back a test double.

For the `uploadImage` method, we want to test that the method returns the created listing image from the data passed into the DTO:
...
describe(`ListingService`, () => {
  let listingService: ListingService
  let databaseService: DeepMockProxy<DatabaseService>
  let googleCloudStorage: DeepMocked<StorageService>
  ...
  beforeEach(async () => {
    ...
    listingService = module.get<ListingService>(ListingService)
    databaseService = module.get(DatabaseService)
    googleCloudStorage = module.get(StorageService)
  })
  ...
  describe(`uploadImage`, () => {
    it(`should return the created listing image`, async () => {
      ...
      googleCloudStorage.uploadFile.mockResolvedValueOnce(publicUrl)
      const mockResponse = {
        id: 1,
        listingId,
        url: publicUrl,
      ...
      // Act
      const result = await listingService.uploadImage({
        listingId,
        mimeType: `image/jpeg`,
...
Let's now get to integration tests.

## Integration testing

With our integration tests, we want to:

- Test using real dependencies and real state
- Test to make sure any side effects are working as expected
- Test the end result of the method

Typically, most (if not all) of your business logic is in the service layer. In this project, the `ListingService` is the service layer and where the business logic happens.

Before diving in, you'll need to make sure the `.env.test` file is up-to-date.

Otherwise, when your test setup runs, you'll get some errors due to missing environment variables.

If you've been following along in this tutorial, you'll just need to add the Google Cloud environment variables. As explained later on, we'll actualy be mocking the Google Cloud Service in our integration tests, so you can just provide some test values in yout `.env.test` file:

```bash
# ...

GCS_PROJECT_ID="test"
GCS_CLIENT_EMAIL="test"
GCS_PRIVATE_KEY="test"
GCS_BUCKET_LISTING_IMAGES="test"
```

Let's write some integration tests for this service layer so the business logic is tested.

In the `/listing` directory, add a new file called `listing.service.int-spec.ts` and add the following:
...
This test file leverages the setup we build in the NestJS starter repo to get access to the NestJS application instance by importing `app`. This instance uses real dependencies (it's own dedicated Docker compose setup). We then use this to get access to the `ListingService` and `DatabaseService` instances to:

- Create a listing using the `ListingService`
- Get the created listing from the database using the `DatabaseService`
- Assert that the created listing is the same as the listing returned from the `ListingService`

The payload object we used in the test we're going to need again, so let's create a utility function to generate this payload. In the `/listing` directory, add a new directory called `__tests__` and place a file called `test-utils.ts` inside:
...
You can then update the test to use this function:

```ts
import { app } from '../../../test/setup';
import { DatabaseService } from '../../database/database.service';
import { generateCreateListingPayload } from './__tests__/test-utils';
import { ListingService } from './listing.service';
...
The test we just added for the `create()` method had no images - let's add another test for the `create()` method with images.

Now it's worth mentioning the Google Cloud Service - should we use the real Google Cloud Service in our integration tests or mock it?

Typically, when interacting with 3rd party (network based) APIs, it's generally considered best practice to mock them instead of using the real API. Here are some reasons why:

- Cost: Every upload or other operation on GCS might incur costs.
- Reliability: Your tests could fail if there are any issues with GCS or network connectivity at the time of running the tests.
- Cleanup and Side Effects: As you mentioned, if you are uploading real files, you'd need to handle deleting them afterward, making sure you don't accidentally delete something important or leave orphaned data.

So let's update our testing set up so all integration and e2e tests use a mocked Google Cloud Service.

Open up the `/test/setup.ts` file and let's provide a mock Storage Service:
...
import { StorageService } from '../src/services/google-cloud/storage/storage.service'
import { createMock } from '@golevelup/ts-jest'
...
let googleCloudStorage: StorageService
...
beforeAll(async () => {
  ...
  // google cloud mock
  googleCloudStorage = moduleFixture.get(StorageService)
  const mockUrl = `https://storage.googleapis.com/bucket/random-uid`
  googleCloudStorage.uploadFile = jest.fn().mockResolvedValue(mockUrl)
...
Here, we've used the `createMock` method from the `@golevelup/ts-jest` package to create a mock `StorageService` and then used the `useValue` property to provide this mock to the module.

We've then used the `mockResolvedValue` method to mock the `uploadFile` method to return a mock url each time it's called.

Let's now go back to the integration test file and add a test for the `create()` method with images.

Let's use some real images in the test, so inside the `__test__` directory we made earlier, add a new directory called `images` and place 3 images inside (any images will do).
...
describe(`ListingService Integration Tests`, () => {
  ...
  describe(`create`, () => {
    ...
    it(`should create a listing with images`, async () => {
      ...
      // grab a local static test image from the __tests__/images folder
      const imagePath = join(__dirname, `./__tests__/images/image_1.jpg`)
...
This test is similar to the previous test except we're passing in an image to the `create()` method. So we should update this test to assert that the listing has an image associated with it.

The thing is, however, is that the file upload and association with the listing happens in the queue.

Let's update our test to wait for the queue to finish processing the job before asserting that the listing has an image associated with it.

To do this, we need to access the respective queue and wait for it to finish processing the job - let's update the `/test/setup.ts` file to do this:
...
export { app, server, listingQueue } // <-- include listingQueue in export
```

Let's now update the test to:

- Wait until the queue has finished processing the job
- Assert that the listing has an image associated with it and has stored the image record in the database
...
describe(`ListingService Integration Tests`, () => {
  ...
  describe(`create`, () => {
    ...
    it(`should create a listing with images`, async () => {
      ...
      expect(listing).toEqual(persistedListing)
      expect(persistedImages.length).toBe(1)
    })
  })
})
```

We've used the `getJobs` method from the `Queue` instance to get all the jobs that are currently in the `active` state. We then wait until there are no more jobs in the `active` state before asserting that the listing has an image associated with it and has stored the image record in the database.

Run `pnpm test:e2e` in the terminal and you should see all the tests passing!

In our `ListingService`, we also have a `uploadImage()` method - this method is called by the `ListingConsumer` when a job is processed which we've actualy covered in the test above by waiting for the queue to finish processing the job.

So let's now move onto some end-to-end tests.
...
- some unit tests across the app for the pipes and services
- some integration tests for the `ListingService` which tests the end result and also the side effects (i.e. the database).

So what's left to test? Well, we haven't tested the API endpoint to create a listing. We've tested the service methods but we haven't tested the API endpoints that call those service methods.

This is where end-to-end testing comes in.

End to end tests are great for:

- testing the validation layers are working correctly (i.e. the DTOs and pipes)
- testing the response of the API endpoints (i.e. the HTTP response code and response body) are what we expect

☝️ You might be thinking - if we add unit tests to the `ListingController`, won't that cover the same tests as the end-to-end tests? Well, the controller methods won't have access to the validation layers, so it's good to have end-to-end tests to test the validation layers. Also, the controller methods just call the respective service methods (which we've already added tests for).
...
With all that said, let's consider what we want to test:

- Test that the `POST /listing` endpoint returns a 201 status code and the created listing
- Test that the `POST /listing` endpoint returns a 400 status code when the DTO payload is invalid
- Test that the `POST /listing` endpoint returns a 400 status code when the file validation fails
...
Let's add a new `describe` block for the `ListingController` and let's add a 1st test to ensure a valid payload returns a 201 status code and the created listing:
...
describe(`ListingController (e2e)`, () => {
  describe(`POST /listing`, () => {
    ...
    it(`should return 201 and return created listing`, () => {
      const imagePath = join(__dirname, `../src/modules/listing/__tests__/images/image_1.jpg`)
...
Let's next add a couple of examples to test that the `POST /listing` endpoint returns a 400 status code when the DTO payload is invalid:
...
And finally, let's do some testing for the file validation:
...
describe(`ListingController (e2e)`, () => {
  describe(`POST /listing`, () => {
    ...
    it(`should return 400 when a non png/jpg/jpeg file is uploaded`, () => {
      const imagePath = join(__dirname, `../src/modules/listing/__tests__/test-utils.ts`)
      return request(server)
        .post(`/listing`)
        .field(`label`, label)
        .field(`addressLine1`, addressLine1)
