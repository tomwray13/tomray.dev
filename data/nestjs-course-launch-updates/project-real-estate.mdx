...
export class ListingProducer {
  ...
  async createListingImage(payload: string) {
...
export class ListingConsumer {
  ...
  async createListingImage(job: Job<string>) {
...
@Module({
  imports: [BullModule.registerQueue({ name: LISTING_QUEUE }), DatabaseModule],
  controllers: [ListingController],
  providers: [ListingService, ListingProducer, ListingConsumer], // <-- add producer and consumer
})
export class ListingModule {}
...
## Using file paths from disk with uploading to Google Cloud Storage
