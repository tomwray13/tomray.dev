...
![How to build a NestJS Docker image for production](/static/images/nestjs-caching-redis/nestjs-caching-redis.png)
...
In this tutorial we'll use the [Pokemon API](https://pokeapi.co/) to demonstrate how to leverage some different caching strategies. Checkout the [Github repo](https://github.com/tomwray13/nestjs-caching) if you'd like to jump straight into the code!
...
When interacting with any module in NestJS, you'll need to import it into respective module. For example, if you have a `Pokemon` module with a controller and service and you'd like to use caching in this module, then you'd need to import (and register) the caching module into the `pokemon.module.ts` file like we've done above in the `app.module.ts` file.
...
Let's say we have a service which calls an external API and the response from this API isn't so dynamic (i.e. it doesn't change very often).
...
To use Redis instead of the in memory cache, we'll need to update the cache module configuration and also setup a local Redis server.
...
You can confirm in your GUI of choice (I like [TablePlus](https://tableplus.com/)) by sending a request to your NestJS app where caching is used and you'll see the data is now persisisted:
...
We'll use the [NestJS config module](https://docs.nestjs.com/techniques/configuration) to take care of this.
...
Then in the `app.module.ts` file, import the config module:
...
@Module({
  imports: [
    ...
    PokemonModule,
    ConfigModule.forRoot() // import the config module,
  ],
})
export class AppModule {}
...
You could now update the Redis config values by using `process.env` like this:
...
However, it's better practice to import them from a custom config file and using the NestJS config module.

That's a little beyond this scope of this tutorial, but I'll be publishing a more in-depth tutorial on how to do this in the future.
...
In your local env file, these additional properties (`username` and `password`) can be empty strings as the Docker container does not require the authentication.
