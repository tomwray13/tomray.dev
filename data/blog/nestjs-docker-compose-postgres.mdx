...
In this tutorial, we're going to use [Docker Compose](https://docs.docker.com/compose/) to create a local development environment for NestJS and Postgres with hot reloading.
...
The local Postgres server will now be running as well as the NestJS app. If you're using the [Docker extension in Visual Studio Code](https://code.visualstudio.com/docs/containers/overview), you will be able to see them both:
...
However, if you (or your colleagues) are working on a Macbook/Mac on the M1 chip then you'll need to update your Dockerfile to the following:
