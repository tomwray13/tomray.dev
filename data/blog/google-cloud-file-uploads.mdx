...
2. [Integrating the graphql-upload package](#integrate-graphql-upload-into-apollo-server)
3. [Throwing an error if the file is too big](#throwing-an-error-if-the-file-is-too-big)
4. [Defining a unique file name](#defining-a-unique-file-name)
5. [Setting up Google Cloud Storage in your project](#setting-up-google-cloud-storage-in-your-project)
6. [Upload the file to your Google Cloud Storage Bucket](#upload-the-file-to-your-google-cloud-storage-bucket)
...
The code `bucket.file(filename).createWriteStream({...})` is where the magic happens. We're importing `bucket` which we defined in our config file and then creating a writeable stream against a file in this bucket, passing in the `filename` (which defines the name of the file that's being uploaded).
