...
- Leave the audiences set to 'Default audience'
...
- And `/attribute.repository/${REPO}` (You'll want to replace `${REPO}` with your github repo using format `username/repo`. For example, mine would be `tomwray13/nest-cloud-run`)
...
There are some helpful comments in the workflow which explains the setup required. We've already covered this setup in the previous steps, but it's worth reading to ensure everything is setup correctly.
...
The `WIF_SERVICE_ACCOUNT` is the email address of the service account you created in a previous step above.
